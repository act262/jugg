...
- 每次检测到 C/C++ 源码变化都会执行 native task；产物内容校验只避免重复写入 APK，不跳过 native 编译。同一个 native 模块 merge 目录里可能有多个 `.so`：重新打包 APK 时会带上该目录全部 `.so`；SO 热更新只把内容 hash 相对上一轮已变的 `.so` 推到设备，未变更的 sibling 仍用设备上已有的文件；没有上一轮记录时本轮全部 push。
