...
- Each detected C/C++ source change runs the native task. Artifact content checks only avoid writing identical output back to the APK; they do not skip native compilation. One native module merge directory can contain several `.so` files. Repackaging the APK includes every library in that directory; SO hot update pushes only the libraries whose content hash changed since the last round, and unchanged siblings keep using the copy already on the device. If there is no previous-round record, this round pushes every library.
