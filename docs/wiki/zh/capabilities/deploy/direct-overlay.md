...
| `run-as`、普通 shell、root adbd 和非交互 `su` 全部不可用 | 兼容 payload 支持，普通 payload 先转兼容部署 | 把兼容 payload 暂存到 `/data/local/tmp/jugg/rootless-compat`，由 App 在下次启动时导入，不推送任何 JVMTI agent |
...
普通 shell、root adbd 和非交互 `su` 也全部不可用时，Jugg 不再尝试写入 App sandbox：普通的 class、资源和 assets 变更先转成兼容部署，再把兼容 payload 暂存到 shell 可写的 `/data/local/tmp/jugg/rootless-compat/<package>/<requestId>/`，只重启 App 一次，由 App 在启动早期导入 `code_cache/.overlay`。这条路径不推送、复制或 attach 任何 JVMTI agent，App 确认导入成功前不会推进 deployment cache、部署历史和文件状态；导入失败或超时会保留旧 overlay 并明确失败。
